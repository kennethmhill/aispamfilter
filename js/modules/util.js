...
export function interpolate_rgb(color1, color2, percentage) {
    return color1.map((channel1, index) => {
        ...
        return Math.min(255, Math.max(0, inter_channel));
    });
}
...
export function update_threshold(m, t){
    m.each((i,e) => { $('[data-email-id="'+e.id+'"]').find('.spam_prob').css({ background: this.get_email_colors(e.spam_prob, t)[0], border: '1px solid '+this.get_email_colors(e.spam_prob, t)});});
}
...
export function get_trimmed_content(content){
    return $('<div>').html(DOMPurify.sanitize(content)).text().substring(0, 500);
}
