...
export class Model {
    ...
    async load_model() {
        try {
            if (!this.model) this.model = await tf.loadLayersModel(this.model_json);
            return Promise.resolve(this.model);
        } catch (error) { return Promise.reject(new Error('Failed to load the model. Error: ' + error.message)) ;}
    }

    // words -> tokens -> tensor
    tokenize(text) {
        const words = text.split(' '); // split into an word array
        ...
        return tf.tensor2d([paddedTokens], [1, this.enc_size]);
    }
    ...
    async predict(message) {
        try {    
            ...
            return data[1];
        } catch (error) { throw new Error('Failed to predict spam probability1. Error: ' + error.message);}
    }
    
    is_spam(prob) { return prob > this.threshold;}
}